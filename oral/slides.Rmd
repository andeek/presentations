...
$$\gamma(\boldsymbol \theta) = \sum\limits_{\boldsymbol x \in \mathcal{C}^{H + V}}\exp\left(\sum\limits_{i = 1}^V \sum\limits_{j=1}^H \theta_{ij} v_i h_j + \sum\limits_{i = 1}^V\theta_{v_i} v_i + \sum\limits_{j = 1}^H\theta_{h_j} h_j\right)$$.
...
The highly flexible nature of a RBM (having as it does $H + V + H V$ parameters) makes at least three kinds of potential model impropriety of concern, *degeneracy*, *instability*, and *uninterpretability*.
...
RBMs often fail to generate data with realistic variability and thus an unsatisfactory conceptualization of the data generation process. Additionally, we find that RBMs easily exhibit instability in the parameter space. In practice, this is seen when a single pixel change in an image results in a wildly different classification. Such model impropriety issues have been documented in in RBMs (@li2014biclustering), as well as other deep architectures (@szegedy2013intriguing, @nguyen2014deep).

# Near-degeneracy

\begin{definition}[Model Degeneracy]
There is a disproportionate amount of probability placed on only a few elements of the sample space, $\mathcal{C}^{H + V}$, by the model.
\end{definition}

RBM models exhibit *near-degeneracy* when random variables in the neg-potential function 
$$Q_{\boldsymbol \theta}(\boldsymbol x) = \sum\limits_{i = 1}^V \sum\limits_{j=1}^H \theta_{ij} v_i h_j + \sum\limits_{i = 1}^V\theta_{v_i} v_i + \sum\limits_{j = 1}^H\theta_{h_j} h_j,
$$ 
have a mean vector $\mu(\boldsymbol \theta)$ close to the boundary of the convex hull of $\mathcal{T} = \{t(x): x \in \mathcal{X}\}$ (@handcock2003assessing), where $t(\boldsymbol x) = \{v_1, \dots, v_V, h_1, \dots, h_H, v_1 h_1, \dots, v_V h_H \}$ and the mean parameterization on the model parameters, $\mu(\boldsymbol \theta)$ is

---

\begin{align*}
\mu(\boldsymbol \theta) &= \text{E}_{\boldsymbol \theta} t(\boldsymbol X)  \\
&= \sum\limits_{\boldsymbol x \in \mathcal{C}^{H + V}} \left\{ t(\boldsymbol x) f_{\boldsymbol \theta} (\boldsymbol x) \right\} \\
&= \sum\limits_{\boldsymbol x \in \mathcal{C}^{H + V}} \left\{ t(\boldsymbol x)\dfrac{\exp\left(\sum\limits_{i = 1}^V \sum\limits_{j=1}^H \theta_{ij} v_i h_j + \sum\limits_{i = 1}^V\theta_{v_i} v_i + \sum\limits_{j = 1}^H\theta_{h_j} h_j\right)}{\sum\limits_{\boldsymbol x \in \mathcal{C}^{H + V}}\exp\left(\sum\limits_{i = 1}^V \sum\limits_{j=1}^H \theta_{ij} v_i h_j + \sum\limits_{i = 1}^V\theta_{v_i} v_i + \sum\limits_{j = 1}^H\theta_{h_j} h_j\right)}\right\}. \\
\end{align*}
...
\begin{align*}
\lim\limits_{V \rightarrow \infty} \frac{1}{V} ELPR(\boldsymbol \theta_V) = \infty.
\end{align*}
where 
\begin{align}
\label{eqn:elpr}
ELPR(\boldsymbol \theta_V) = \log \left[\frac{\max\limits_{(v_1, \dots, v_V) \in \mathcal{C}^V}P_{\boldsymbol \theta_V}(v_1, \dots, v_V)}{\min\limits_{(v_1, \dots, v_V) \in \mathcal{C}^V}P_{\boldsymbol \theta_V}(v_1, \dots, v_V)}\right]
\end{align}
\end{definition}
...
$$
\Delta(\boldsymbol \theta_V) \equiv \max \left\{\log \frac{P_{\boldsymbol \theta_V}(v_1, \dots, v_V)}{P_{\boldsymbol \theta_V}(v_1^*, \dots, v_V^*)} \right\},
$$

where $(v_1, \dots, v_V) \& (v_1^*, \dots, v_V^*) \in \mathcal{C}^V$ differ by exactly one component


\begin{result}
\label{prop:instab}
Let $C > 0$ and $V \ge 1$. If $\frac{1}{V}\text{ELPR}(\boldsymbol \theta_V) > C$, then $\Delta(\boldsymbol \theta_V) > C$.
\end{result}
...
\small
\begin{align*}
M_{\epsilon, \boldsymbol \theta_V} &\equiv \left\{\boldsymbol v \in \mathcal{C}^V: \log P_{\boldsymbol \theta_V}(\boldsymbol v)  > (1-\epsilon)\max\limits_{\boldsymbol v^*}P_{\boldsymbol \theta_V}(\boldsymbol v^*) + \epsilon\min\limits_{\boldsymbol v^*}P_{\boldsymbol \theta_V}(\boldsymbol v^*) \right\}
\end{align*}
...
To numerically investigate unstable RBM model sequences, it is useful to define a computationally tractable lower bound on (\ref{eqn:elpr}) given by
$$
\frac{1}{V}R(\boldsymbol \theta) \text{ for } R(\boldsymbol \theta) \equiv \max_{\boldsymbol v} \max_{\boldsymbol h}Q_{\boldsymbol \theta}(\boldsymbol x) - \min_{\boldsymbol v}\max_{\boldsymbol h}Q_{\boldsymbol \theta}(\boldsymbol x) - H\log 2.
$$
That is, it holds that, in (\ref{eqn:elpr}),
\begin{align}
\frac{1}{V} \text{ELPR}(\boldsymbol \theta) \ge \frac{1}{V}R(\boldsymbol \theta). \label{eqn:lowerbnd}
\end{align}
(A proof of (\ref{eqn:lowerbnd}) appears later.)

# Uninterpretability

\begin{definition}[Uninterpretability]
Characterized by marginal mean-structure (controled by main effect parameters $\theta_{v_i}, \theta_{h_j}$) not being maintained in the model due to dependence (interaction parameters $\theta_{ij}$) (\cite{kaiser2007statistical}).
\end{definition}
...
\begin{align*}
\text{E}\left[\boldsymbol X | \boldsymbol \theta\right] &= \sum\limits_{\boldsymbol x \in \mathcal{C}^{H + V}} \boldsymbol x f_{\boldsymbol \theta}(\boldsymbol x) \\
&= \sum\limits_{\boldsymbol x \in \mathcal{C}^{H + V}} \boldsymbol x \frac{\exp\left(\sum\limits_{i = 1}^V \sum\limits_{j=1}^H \theta_{ij} v_i h_j + \sum\limits_{i = 1}^V\theta_{v_i} v_i + \sum\limits_{j = 1}^H\theta_{h_j} h_j\right)}{\sum\limits_{\boldsymbol x \in \mathcal{C}^{H + V}}\exp\left(\sum\limits_{i = 1}^V \sum\limits_{j=1}^H \theta_{ij} v_i h_j + \sum\limits_{i = 1}^V\theta_{v_i} v_i + \sum\limits_{j = 1}^H\theta_{h_j} h_j\right)} \\
~\\
\text{E}\left[\boldsymbol X | \boldsymbol \theta^*\right] &= \sum\limits_{\boldsymbol x \in \mathcal{C}^{H + V}} \boldsymbol x \frac{\exp\left(\sum\limits_{i = 1}^V \theta_{v_i} v_i + \sum\limits_{j = 1}^H\theta_{h_j} h_j\right)}{\sum\limits_{\boldsymbol x \in \mathcal{C}^{H + V}}\exp\left(\sum\limits_{i = 1}^V\theta_{v_i} v_i + \sum\limits_{j = 1}^H\theta_{h_j} h_j\right)} 
\end{align*}
...
\caption{The convex hull of the "statistic space" $\mathcal{T} = \{t(\boldsymbol x): \boldsymbol x \in \mathcal{X}\} = \{(v_1, h_1, v_1 h_1): v_1, h_1 \in \mathcal{A}\}$ in three dimensions for the toy RBM with one visible and one hidden node for $\mathcal{A} = \{0,1\}$ (left) and $\mathcal{A} = \{-1,1\}$ (right) data encoding. The convex hull of $\mathcal{T} \subset \mathcal{A}^3$ does not fill the unit cube $[0,1]^3$ (left), but does better with $[-1,1]^3$ (right).}
\end{figure}

# The center of the universe

- For the $\mathcal{C} = \{-1, 1 \}$ encoding of $\mathcal{V}$ and $\mathcal{H}$, the origin is the center of the parameter space
...
We prove the contrapositive. Suppose that $\Delta(\boldsymbol \theta_V) \le C$ holds for some $C > 0$. Under the RBM model for visibles, $P_{\boldsymbol \theta_V}(\boldsymbol v) > 0$ holds for each outcome $\boldsymbol v \in \mathcal{C}^V$. Let $\boldsymbol v_{min} \equiv \argmin\limits_{\boldsymbol v \in \mathcal{C}^V}P_{\boldsymbol \theta_V}(\boldsymbol v)$ and $\boldsymbol v_{max} \equiv \argmax\limits_{\boldsymbol v \in \mathcal{C}^V}P_{\boldsymbol \theta_V}(\boldsymbol v)$. Note there exists a sequence $\boldsymbol v_{min} \equiv \boldsymbol v_0, \boldsymbol v_1, \dots, \boldsymbol v_k \equiv \boldsymbol v_{max}$ in $\mathcal{C}^V$ of component-wise switches to move from $\boldsymbol v_{min}$ to $\boldsymbol v_{max}$ in the sample space (i.e. $\boldsymbol v_i, \boldsymbol v_{i + 1} \in \mathcal{C}^V$ differ by exactly $1$ component for $i = 0, \dots, k$) for some integer $k \in \{0, 1, \dots, V\}$. Then
\begin{align*}
\log\left[\frac{P_{\boldsymbol \theta_V}(\boldsymbol v_{max})}{P_{\boldsymbol \theta_V}(\boldsymbol v_{min})}\right] &= \left|\sum\limits_{i = 1}^k\log\left(\frac{P_{\boldsymbol \theta_V}(\boldsymbol v_i)}{P_{\boldsymbol \theta_V}(\boldsymbol v_{i-1})}\right)\right| \\
& \le \sum\limits_{i = 1}^k\left|\log\left(\frac{P_{\boldsymbol \theta_V}(\boldsymbol v_i)}{P_{\boldsymbol \theta_V}(\boldsymbol v_{i-1})}\right)\right| \\
& \le k \Delta(\boldsymbol \theta_V) \le VC
\end{align*}
using $k \le V$ and $\Delta(\boldsymbol \theta_V) \le C$.$\Box$

# Appendix: Proof of Result \ref{prop:degen}

\scriptsize 

Define $\boldsymbol v_{max}$ and $\boldsymbol v_{min}$ as in the proof of Proposition \ref{prop:instab}. Fix $0 < \epsilon < 1$ Then, $\boldsymbol v_{max} \in M_{\epsilon, \boldsymbol \theta_V}$, so $P_{\boldsymbol \theta_V}(M_{\epsilon, \boldsymbol \theta}) \ge P_{\boldsymbol \theta_V}(\boldsymbol v_{max})$. If $\boldsymbol v \in \mathcal{C}^V \setminus M_{\epsilon, \boldsymbol \theta_V}$, then by definition $P_{\boldsymbol \theta_V}(\boldsymbol v) \le [P_{\boldsymbol \theta_V}(\boldsymbol v_{max})]^{1-\epsilon}[P_{\boldsymbol \theta_V}(\boldsymbol v_{min})]^{\epsilon}$ holds so that
\begin{align*}
1-P_{\boldsymbol \theta_V}(M_{\epsilon, \boldsymbol \theta_V}) &= P_{\boldsymbol \theta_V}(M_{\epsilon, \boldsymbol \theta_V}^C) \\
& = \sum\limits_{\boldsymbol v \in \mathcal{C}^V \setminus M_{\epsilon, \boldsymbol \theta_V}}P_{\boldsymbol \theta_V}(\boldsymbol v) \\
& \le (2^V)[P_{\boldsymbol \theta_V}(\boldsymbol v_{max})]^{1-\epsilon}[P_{\boldsymbol \theta_V}(\boldsymbol v_{min})]^{\epsilon}
\end{align*}
Then,
\begin{align*}
\frac{1}{V}\log\left[\frac{P_{\boldsymbol \theta_V}(M_{\epsilon, \boldsymbol \theta_V})}{1-P_{\boldsymbol \theta_V}(M_{\epsilon, \boldsymbol \theta_V})}\right] & \ge \frac{1}{V} \log\left[\frac{P_{\boldsymbol\theta_V}(\boldsymbol v_{max})}{(2^V)[P_{\boldsymbol \theta_V}(\boldsymbol v_{max})]^{1-\epsilon}[P_{\boldsymbol \theta_V}(\boldsymbol v_{min})]^{\epsilon}}\right] \\
&= \frac{\epsilon}{V} \log\left[\frac{P_{\boldsymbol \theta_V}(\boldsymbol v_{max})}{P_{\boldsymbol \theta_V}(\boldsymbol v_{min})}\right] - \log 2 \rightarrow \infty
\end{align*}
...
\begin{align*}
\log\left[\frac{\max\limits_{\boldsymbol v \in \mathcal{C}^V}P_{\boldsymbol \theta}(\boldsymbol v)}{\min\limits_{\boldsymbol v \in \mathcal{C}^V}P_{\boldsymbol \theta}(\boldsymbol v}) \right]  &= \log\left[\frac{\max\limits_{\boldsymbol v}\sum\limits_{\boldsymbol h \in \mathcal{C}^H} \exp\left(\sum\limits_{i = 1}^V \theta_{v_i}v_i + \sum\limits_{j = 1}^H \theta_{h_j}h_j + \sum\limits_{i = 1}^V\sum\limits_{j = 1}^H \theta_{ij}v_i h_j\right)}
{\min\limits_{\boldsymbol v}\sum\limits_{\boldsymbol h \in \mathcal{C}^H} \exp\left(\sum\limits_{i = 1}^V \theta_{v_i}v_i + \sum\limits_{j = 1}^H \theta_{h_j}h_j + \sum\limits_{i = 1}^V\sum\limits_{j = 1}^H \theta_{ij}v_i h_j\right)}\right] \\
& \ge \log\frac{\max\limits_{\boldsymbol v}\max\limits_{\boldsymbol h \in \mathcal{C}^H}\exp\left(\sum\limits_{i = 1}^V \theta_{v_i}v_i + \sum\limits_{j = 1}^H \theta_{h_j}h_j + \sum\limits_{i = 1}^V\sum\limits_{j = 1}^H \theta_{ij}v_i h_j\right)}
{\min\limits_{\boldsymbol v}2^H\max\limits_{\boldsymbol h \in \mathcal{C}^H}\exp\left(\sum\limits_{i = 1}^V \theta_{v_i}v_i + \sum\limits_{j = 1}^H \theta_{h_j}h_j + \sum\limits_{i = 1}^V\sum\limits_{j = 1}^H \theta_{ij}v_i h_j\right)} \\
&  = \max\limits_{\boldsymbol v}\max\limits_{\boldsymbol h \in \mathcal{C}^H}\left\{\sum\limits_{i = 1}^V \theta_{v_i}v_i + \sum\limits_{j = 1}^H \theta_{h_j}h_j + \sum\limits_{i = 1}^V\sum\limits_{j = 1}^H \theta_{ij}v_i h_j\right\} - \\
&  \qquad \min\limits_{\boldsymbol v}\max\limits_{\boldsymbol h \in \mathcal{C}^H}\left\{\sum\limits_{i = 1}^V \theta_{v_i}v_i + \sum\limits_{j = 1}^H \theta_{h_j}h_j + \sum\limits_{i = 1}^V\sum\limits_{j = 1}^H \theta_{ij}v_i h_j\right\} -H\log2 \\
&  \equiv R(\boldsymbol \theta)
\end{align*}
using above the monotonicity of $\log(\cdot)$ and positivity of $\exp(\cdot)$.
