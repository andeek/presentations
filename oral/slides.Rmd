...
$$ 
have a mean vector $\mu(\boldsymbol \theta)$ close to the boundary of the convex hull of $\mathcal{T} = \{t(x): x \in \mathcal{C}^{H + V}\}$ (@handcock2003assessing), where $t(\boldsymbol x) = \{v_1, \dots, v_V, h_1, \dots, h_H, v_1 h_1, \dots, v_V h_H \}$ and the mean parameterization on the model parameters, $\mu(\boldsymbol \theta)$ is
...
Characterized by marginal mean-structure (controlled by main effect parameters $\theta_{v_i}, \theta_{h_j}$) not being maintained in the model due to dependence (interaction parameters $\theta_{ij}$) (\cite{kaiser2007statistical}).
\end{definition}
