...
A parametric form for probabilities corresponding to a potential vector of states of each node
...
There is a disproportionate amount of probability placed on only a few elements of the sample space, $\mathcal{X} = \{-1,1\}^{H + V}$, by the model.
\end{definition}
...
\begin{align}
\label{eqn:elpr}
ELPR(\boldsymbol \theta_V) = \log \left[\frac{\max\limits_{(v_1, \dots, v_V) \in \{-1,1\}^V}P_{\boldsymbol \theta_V}(v_1, \dots, v_V)}{\min\limits_{(v_1, \dots, v_V) \in \{-1,1\}^V}P_{\boldsymbol \theta_V}(v_1, \dots, v_V)}\right]
\end{align}
\end{definition}

S-unstable RBM models are undesirable for several reasons. Again, one is that small changes in data can lead to overly-sensitive changes in probability. 
...
\begin{result}
\label{prop:instab}
If $\frac{1}{V}\text{ELPR}(\boldsymbol \theta_V) > C$, then $\Delta(\boldsymbol \theta_V) > C$.
\end{result}

If the quantity (\ref{eqn:elpr}) is too large, then the RBM model will exhibit large probability shifts for very small changes in the data configuration.

# Tie to degeneracy

Unstable RBM models are connected to degenerate models (placing all probability on a small pieace of the sample space). Define a modal set 
\begin{align*}
M_{\epsilon, \boldsymbol \theta_V} &\equiv \left\{\boldsymbol v \in \{-1,1\}^V: \log P_{\boldsymbol \theta_V}(\boldsymbol v)  > (1-\epsilon)\max\limits_{\boldsymbol v^*}P_{\boldsymbol \theta_V}(\boldsymbol v^*) + \epsilon\min\limits_{\boldsymbol v^*}P_{\boldsymbol \theta_V}(\boldsymbol v^*) \right\}
\end{align*}
...
\begin{result}
\label{prop:degen}
For an unstable RBM model, and for any given $0 < \epsilon < 1$, $P_{\boldsymbol \theta_V}\left((v_1, \dots, v_V) \in M_{\epsilon, \boldsymbol \theta_V}\right) \rightarrow 1$ holds as $V \rightarrow \infty$.
\end{result}
